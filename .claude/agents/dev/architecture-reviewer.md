...
- ❌ **보안 1차 사안을 본인 영역으로 분류** — 토큰/credential 노출, XSS, 권한 우회 같은 사안은 _레이어 위반의 부산물처럼 보일지라도_ `security-reviewer` 1차 담당. 본인은 "📌 범위 밖"에 `[security-reviewer 권장]`으로 한 줄만 표기하고, 🚨/⚠️ 항목으로 격상하지 않는다. 본인의 🚨/⚠️ 영역은 *순수 레이어 위반 / 추상화 / 중복 / SOLID*에 한정한다.
